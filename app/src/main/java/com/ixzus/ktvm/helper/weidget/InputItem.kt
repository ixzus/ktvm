 ...
 */
class InputItem(
        var imageSizeDip: Int = 20 //头部图片控件大小
        , var headImageResource: Int = 0 //头部图片资源id
        , var rightCheckImageResource: Int = R.drawable.ic_right //正确检查图片id
        , var wrongCheckImageResource: Int = R.drawable.ic_wrong //错误检查图片id
        , var deleteImageResource: Int = R.drawable.ic_del //删除图片id
        , var testSizeSp: Float = 14f //输入框字体大小
        , var inputTypeEdit: Int = InputType.TYPE_CLASS_TEXT //输入框输入的内容样式
        , var hintResourceInt: Int //提示文字
        , var linearBackgroundResource: Int = R.drawable.input_item_bg //背景资源id
        , var editVerifyCondition: (text: String) -> Boolean)  //输入框 正确错误检查lambda表达式
{
    lateinit var inputEdit: LinearLayout
    lateinit var headImage: ImageView
    lateinit var edit: EditText
    lateinit var rightCheck: ImageView
    lateinit var wrongChcek: ImageView
    ...
    fun showInputCheckIcon(isShowRightCheck: Boolean = false, isShowWrongCheck: Boolean = false, isShowDelete: Boolean = false) {
        ...
        wrongChcek.visibility = if (isShowWrongCheck) View.VISIBLE else View.INVISIBLE
        delete.visibility = if (isShowDelete) View.VISIBLE else View.INVISIBLE
    }

    fun getInputItem(viewManager: ViewManager): LinearLayout {
        inputEdit = with(viewManager) {
            linearLayout {
                ...
                if (headImageResource != 0) {
                    headImage = imageView(headImageResource).lparams(width = dip(imageSizeDip), height = dip(imageSizeDip)) { gravity = Gravity.CENTER_VERTICAL }
                }
                edit = editText {
                    //                    textSize = sp(testSizeSp).toFloat()
                    textSize = testSizeSp
                ...
                edit.setOnFocusChangeListener { v: View?, hasFocus: Boolean ->
                    if (hasFocus) {
...
//                        showInputCheckIcon(isShowDelete = true)
                ...
                frameLayout {
                    rightCheck = imageView(rightCheckImageResource) {
                        visibility = View.INVISIBLE
                    }.lparams(width = dip(imageSizeDip), height = dip(imageSizeDip))
                    wrongChcek = imageView(wrongCheckImageResource) {
                        visibility = View.INVISIBLE
                    }.lparams(width = dip(imageSizeDip), height = dip(imageSizeDip))
